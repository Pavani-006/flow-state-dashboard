...
export const Route = createFileRoute("/goals")({
  head: () => ({
    meta: [
      { title: "Goals — Flowdesk" },
      { name: "description", content: "Set, track, and crush your goals." },
    ],
  }),
  component: GoalsPage,
});
    ...
    <div>
            ...
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button
                  onClick={() => {
                    if (draft.title.trim()) {
                      addGoal({ title: draft.title.trim(), deadline: draft.deadline || undefined });
      ...
      {goals.length === 0 ? (
        <EmptyState
          ...
          title="No goals yet"
          description="Start by setting one meaningful goal for this quarter."
        />
      ) : (
                ...
                <CardContent>
                  ...
                  <div className="space-y-1.5">
                    {g.milestones.map((m) => (
                      <label
                        key={m.id}
                        className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-accent/40"
                      >
                        <Checkbox
                          checked={m.done}
                          onCheckedChange={() => toggleMilestone(g.id, m.id)}
                        />
                        <span
                  ...
                  <div className="mt-3 flex gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setNewMilestones({ ...newMilestones, [g.id]: e.target.value })
                      }
                      onKeyDown={(e) => {
