...
export const Route = createFileRoute("/notes")({
  head: () => ({
    meta: [
      { title: "Notes — Flowdesk" },
      { name: "description", content: "Quick notes and pinned thoughts." },
    ],
  }),
  component: NotesPage,
});
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          title="No notes"
          description="Click 'New note' to create your first one."
        />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((n) => (
            <Card
              key={n.id}
              className={`group transition-all hover:shadow-elegant ${n.pinned ? "ring-1 ring-primary/30" : ""}`}
            >
