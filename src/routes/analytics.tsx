...
export const Route = createFileRoute("/analytics")({
  head: () => ({ meta: [{ title: "Analytics — Flowdesk" }, { name: "description", content: "Visualize your productivity trends." }] }),
...
function AnalyticsPage() {
  ...
  const weekly = useMemo(() => Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      ...
      focus: sessions.filter((s) => s.completedAt.slice(0, 10) === k).reduce((a, s) => a + s.durationMin, 0),
    };
  }), [tasks, sessions]);
  ...
  const heatmap = useMemo(() => {
    ...
    for (let i = 41; i >= 0; i--) {
      ...
      const k = d.toISOString().slice(0, 10);
      const count = tasks.filter((t) => t.completedAt?.slice(0, 10) === k).length + sessions.filter((s) => s.completedAt.slice(0, 10) === k).length;
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="text-base">Tasks completed (7d)</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Focus minutes (7d)</CardTitle></CardHeader>
          <CardContent className="h-72 pl-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weekly}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Activity heatmap (6 weeks)</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-flow-col grid-rows-7 gap-1">
              {heatmap.map((d) => (
                <div key={d.date} title={`${d.date}: ${d.count} actions`} className={`h-4 w-4 rounded-sm ${heatColor(d.count)} transition-transform hover:scale-110`} />
              ))}
            </div>
